...
pub fn main() !void {
    ...
    try add_to_array(allocator, &numbers, 1);
    try add_to_array(allocator, &numbers, 10);

    const ns = try numbers.toOwnedSlice(allocator);
...
fn add_to_array(allocator: std.mem.Allocator, arr: *std.ArrayList(u8), n: u8) !void {
    try arr.append(allocator, n);
}

fn add_str(s: []const u8, rest: []const u8) ![]const u8 {
    var inside_buffer: [40]u8 = undefined;
    return try std.fmt.bufPrint(&inside_buffer, "{s}{s}", .{ s, rest });
}

fn add_str_buffer(buffer: []u8, s: []const u8, rest: []const u8) ![]const u8 {
...
fn concat_str_with_allocator(allocator: std.mem.Allocator, first: []const u8, second: []const u8) ![]const u8 {
