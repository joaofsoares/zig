    ...
    for (0..count) |row| {
        ...
        for (0..(row + 1)) |i| {
            if (row == i or i == 0) {
                ...
                arr[i] = mat[row - 1][i - 1] + mat[row - 1][i];
            }
        }
...
test "single row" {
    ...
    const actual = try rows(testing.allocator, 1);
...
test "two rows" {
    ...
    const actual = try rows(testing.allocator, 2);
...
test "three rows" {
    ...
    const actual = try rows(testing.allocator, 3);
...
test "four rows" {
    ...
    const actual = try rows(testing.allocator, 4);
...
test "five rows" {
    ...
    const actual = try rows(testing.allocator, 5);
...
test "six rows" {
    ...
    const actual = try rows(testing.allocator, 6);
...
test "ten rows" {
    ...
    const actual = try rows(testing.allocator, 10);
